...
/* === ADD-ONLY: CalendarApp namespace + proxy state + helpers === */
(function () {
    if (window.CalendarApp) return;

    const state = {};
    Object.defineProperties(state, {
        month: { get: () => window.currentMonth, set: (v) => { window.currentMonth = v; } },
        year: { get: () => window.currentYear, set: (v) => { window.currentYear = v; } }
    });
...
document.addEventListener('DOMContentLoaded', function () {
    /* === ADD-ONLY: Escape closes open modals (no behavior changes otherwise) === */
    document.addEventListener('keydown', function (e) {
        ...
        if (taskEditModal && taskEditModal.style.display === 'block') {
    ...
    document.getElementById('prev-month').addEventListener('click', function () {
        // ADD-ONLY reroute: use CalendarApp proxy state (old code remains below)
        if (window.CalendarApp && window.CalendarApp.state) {
            window.CalendarApp.state.month--;
            if (window.CalendarApp.state.month < 0) {
                window.CalendarApp.state.month = 11;
                window.CalendarApp.state.year--;
            }
            renderCalendar(window.CalendarApp.state.month, window.CalendarApp.state.year);
            return;
        }
    ...
    document.getElementById('next-month').addEventListener('click', function () {
        ...
        if (window.CalendarApp && window.CalendarApp.state) {
            window.CalendarApp.state.month++;
            if (window.CalendarApp.state.month > 11) {
                window.CalendarApp.state.month = 0;
                window.CalendarApp.state.year++;
            }
            renderCalendar(window.CalendarApp.state.month, window.CalendarApp.state.year);
            return;
